...
- Centralize ignore matching across watchers, saves, comparison scans, WebMaster tools, and recursive transfers.
- Prune ignored directories before recursive local or remote traversal and cache compiled glob expressions to reduce scan work.
- Queue only changed files for Sync changed actions instead of recursively retransferring an entire changed folder.
- Debounce Diff Viewer filesystem updates per path so simultaneous edits are retained without forcing a full comparison refresh.
...
- Stop the Diff Viewer progress animation whenever its status is idle or waiting for operations.
- Treat ignored files, ignored folders, and every descendant as out of scope for scans and transfers.
- Repair the malformed notification CSS declaration that caused browsers to discard later dashboard styles.
- Suppress Auto Sync events generated by downloads so a downloaded file is not immediately uploaded again.
- Normalize Windows watcher paths before ignore matching so ignored folders exclude every descendant from Auto Sync.
